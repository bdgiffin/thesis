...
\section{Scope of the Present Work}
	...
	In this work, we define a general framework for \textit{partitioned element methods} (PEM) -- a family of approaches for constructing approximate representations of element shape functions on arbitrary polytopes. These methods are characterized by several distinguishing features:
	\begin{itemize}
		\item[(1)] Elements are discretized (partitioned) into non-overlapping polytopal cells. These cells inform the quadrature rule of the element.
		\item[(2)] The elements' shape functions are defined to be piecewise polynomial in each cell.
		\item[(3)] Nodal shape functions are determined as the solutions to a series of boundary value problems, defined hierarchically on each element and its bounding faces, edges.
	\end{itemize}
	Numerous approaches are possible within this framework; a few of these are explored within the scope of this thesis.
