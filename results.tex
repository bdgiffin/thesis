...
As a brief aside, if the DG-PEM is utilized on the series of quadrilateral meshes shown in Figure \ref{fig:quad_annulus_meshes}, the resulting convergence rates are on par with those of the FEM. This illustrates an important point: the choice of discretization has an impact on the solution accuracy.
...
Without the use of an adequate model for ductile fracture, it is not possible to predict when rupture will occur within the specimen. Nonetheless, an ad hoc metric of 60\% reduction in area will be used to \textit{estimate} the point at which rupture occurs.
...
\begin{tabular}{| c || c | c |}
    ...
    FEM (with enhancement)    & 14.65 & 70.74 \\ \hline
    DG-PEM (with enhancement) & 12.89 & 72.15 \\
    \hline
\end{tabular}
...
Neither simulation comes particularly close to reproducing the experimental results. This may attributable to a number of potential factors: failure to account for the effects of material variability, inadequacy of the chosen linear hardening plasticity model, poorly calibrated model parameters, or coarseness of the meshes that were utilized. Irrespective of these shortcomings, however, it is difficult to rationalize the difference between the results obtained by the FEM and the PEM. 

Interestingly, if the PEM is employed on the hexahedral mesh in Figure \ref{fig:necking_hex_mesh}, the results are comparable with those of the FEM. This observation seems to indicate that the results are sensitive to the particular choice of discretization. Running the problem on a more highly refined mesh (with additional elements through the thickness of the plate) results in much stiffer behavior. The specimen continues to harden well beyond 6 \% elongation, with no onset of necking. This latter observation seems to indicate that some form of locking may be affecting the results on the finer meshes that were considered.
...
\end{equation}
In the above, $u_3$ corresponds to the vertical displacement of a given point on the bottom face of the rod, $\langle \cdot \rangle$ denote the Macaulay brackets, and $p$ is a penalty stiffness parameter. For the problem at hand, an ad hoc value of $p = 6.0E+016$ was chosen (following a process of trial-and-error).
...
\begin{figure}[!h]
    ...
    \begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=3.0in]{figures/taylor_bar_eqps000.pdf}
    			\caption{$t= 0.0 \times 10^{-5}$ sec. \label{fig:taylor_bar_eqps_0}}
    \end{subfigure}
	\begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=3.0in]{figures/taylor_bar_eqps060.pdf}
    			\caption{$t= 1.2 \times 10^{-5}$ sec. \label{fig:taylor_bar_eqps_1}}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=3.0in]{figures/taylor_bar_eqps120.pdf}
    			\caption{$t= 2.4 \times 10^{-5}$ sec. \label{fig:taylor_bar_eqps_2}}
    \end{subfigure}
	\begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=3.0in]{figures/taylor_bar_eqps180.pdf}
    			\caption{$t=3.6 \times 10^{-5}$ sec. \label{fig:taylor_bar_eqps_3}}
    \end{subfigure}
    \caption{Comparison of results between the FEM and PEM models at various times during the analysis, depicting deformed shape and color map values of equivalent plastic strain $\bar{\epsilon}^p$.}
...
\begin{figure}[!h]
  \centering
    \begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=3.0in]{figures/taylor_bar_s12.pdf}
    			\caption{$\sigma_{12}$ \label{fig:taylor_bar_s12}}
    \end{subfigure}
	\begin{subfigure}[b]{0.49\linewidth}
            \centering
            \includegraphics[width=3.0in]{figures/taylor_bar_s22.pdf}
    			\caption{$\sigma_{22}$ \label{fig:taylor_bar_s22}}
    \end{subfigure} \caption{Comparison of stresses within the FEM and PEM models at $t = 3.6 \times 10^{-5}$ seconds. In each diagram, the FEM model occupies the $-X_2$ half-space, and the PEM model occupies the $+X_2$ half-space.}
  \label{fig:taylor_bar_stresses}
\end{figure}

An interesting result occurs if the kinematic enhancement of \cite{Rashid:06} is used on the polyhedral mesh. Specifically, 
...
\begin{table}[!ht]
  \begin{center}
    ...
    FEM (no enhancement)      & 11.37 & 23.04 \\ \hline
    FEM (with enhancement)    & 13.91 & 23.60 \\ \hline
    DG-PEM (no enhancement)   & 13.47 & 23.38 \\
    \hline
    \end{tabular}
