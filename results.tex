...
A handful of studies were conducted to assess the convergence behavior of the DG-PEM. These studies sought to verify that the DG-PEM yields convergent sequences of approximations under mesh refinement. A few relatively simple two- and three-dimensional elasticity problems are considered. The results obtained for the DG-PEM are compared against the results for the classical FEM. Convergence of both linear and quadratic PEM elements is established, and convergence in the incompressible limit is examined for discretizations consisting of hexahedra and arbitrary polyhedra.
...
The arbitrary polyhedral meshes of Figure \ref{fig:poly_annulus_meshes} are less sensitive to the effects of volumetric locking. Optimal rates of convergence are fully restored when the kinematic enhancement is utilized. As a brief aside, if the DG-PEM is utilized on either of the two series of hexahedral meshes shown in Figures \ref{fig:hex_annulus_meshes} and \ref{fig:hexp_annulus_meshes}, the resulting convergence rates are effectively the same as those of the FEM. This illustrates an important point: the particular choice of \textit{discretization} (rather than element formulation) directly impacts sensitivity to volumetric locking.
