...
The two meshes depicted in Figure \ref{fig:patch_test_meshes} are considered. The PEM was used on the polygonal mesh, whereas the quadrilateral mesh consisted of standard isoparametric elements.

\begin{figure}[!h]
    \centering
    \includegraphics[width=4.0in]{figures/patch_test_meshes.pdf}
    	\caption{2D meshes for the linear and quadratic patch tests: (left) polygonal mesh, (right) distorted quadrilateral mesh.}
...
\begin{table}[!ht]
  \begin{center}
    ...
    \hline
    \end{tabular}
    \caption{The effect of interpolation error on patch test error in the DG-PEM, in the absence of a gradient correction scheme.}
    \vspace{-5pt}
...
If the interpolation error $E_1 (\Omega)$ becomes large enough (due to poor conditioning of $\mathbf{J}$), it will dominate any other sources of error incurred in the patch test. Interpolation errors persist even if a gradient correction scheme is employed, and can become particularly troublesome for: thin elements, higher-order elements, or elements with nearly degenerate features.
...
\begin{table}[!ht]
  \begin{center}
    ...
    \hline
           & $||\mathbf{u}^h - \mathbf{u}|| / ||\mathbf{u}||$ & $||\boldsymbol{\sigma}^h - \boldsymbol{\sigma}|| / ||\boldsymbol{\sigma}||$ \\ \hline \hline
    FEM (8-node quadrilaterals) & 4.8968E-003 & 5.2227E-002 \\ \hline
    DG-PEM (no gradient correction) & 9.8535E-002 & 2.8876E-001 \\ \hline
    DG-PEM (symmetric correction) & 3.5935E-008 & 1.4642E-007 \\ \hline
    DG-PEM (nonsymmetric correction) & 4.5498E-009 & 5.5341E-009 \\
    \hline
    \end{tabular}
    \caption{Quadratic patch test results.}
    \vspace{-5pt}
...
\subsection*{Conditioning of Element Stiffness Matrices}

Partitioned element methods allow for the elements to take on virtually arbitrary shape. However, elements with non-convex or degenerate features must be tolerated to the extent that their presence within a finite element mesh should not degrade the overall accuracy of the resulting numerical solution. To this end, this section attempts to characterize the quality of a representative sampling of polygonal PEM elements by examining the eigenvalues of each element's elastic stiffness matrix.
...
As a separate issue from locking, if a given element's local stiffness matrix is poorly conditioned, the global stiffness matrix of a mesh which contains said element may become poorly conditioned, as well. This has negative implications for the degree of solution accuracy that can be obtained due to floating point arithmetic. For implicit solid mechanics applications, the accuracy of any linear solver (whether direct or iterative) will be compromised by poor conditioning of the global stiffness matrix.

For the elements considered, a comparison study was carried out to examine differences in the eigenvalue spectra of elements whose stiffness matrices were computed using different element formulations. Specifically, separate approximations to the harmonic shape functions were obtained for each element using the VETFEM, the CG-PEM, and the DG-PEM. These results were compared against a reference solution for the harmonic shape functions.

For each element, the elastic constants for Young's modulus and Poisson's ratio were chosen as $E = 1.0$ and $\nu = 0.0$, respectively. The choice of $\nu = 0.0$ is deliberate, and aims to decouple the effects of volumetric locking from the effects of geometric locking. The elements were scaled to have roughly unit diameter.
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=3.5in]{figures/concave_element_partitions.pdf}  \caption{Delaunay partitions for the elements A, B, and C introduced in Figure \ref{fig:concave_element_shapes}.}
  \label{fig:concave_element_partitions}
\end{figure}

For the red nodes indicated in Figure \ref{fig:concave_element_shapes}, color plots for the associated harmonic shape functions (and their corresponding approximations) are provided in Figure \ref{fig:concave_element_comparison}.

\begin{figure}[!h]
  \centering
  \includegraphics[width=5.0in]{figures/concave_element_comparison.pdf}  \caption{Comparison of the shape functions computed for the polygonal elements A, B, and C using VETFEM, CG-PEM, and DG-PEM.}
...
\begin{tabular}{| c | c | c | c | c |}
    \hline
DG-PEM & $\alpha_{0\gamma} = 10^{-3}$ & $\alpha_{0\gamma} = 10^{0}$ & $\alpha_{0\gamma} = 10^{3}$ & $\alpha_{0\gamma} = 10^{6}$ \\ \hline
...
The results in table \ref{tab:concave_stiffness_max_min_eigenvalue_parameter_study} warrant a number of observations. The eigenvalue spectrum obtained for the DG-PEM converges to that of the CG-PEM as the value of $\alpha_{0\gamma}$ is increased (provided $\alpha_{1\gamma}$ is sufficiently small). Moreover, the shape functions will themselves converge to the $C^0$ continuous approximations obtained by the CG-PEM. This is an expected result.
...
\subsubsection*{Examination of the Effects of Geometric Degeneracy}
...
As in the previous example, the aforementioned approximations were used in place of the harmonic shape functions, and the elements' elastic stiffness matrices were computed (by numerical quadrature). The eigendecomposition of each element's local stiffness matrix was then determined, with attention paid to the largest and smallest (non-zero) eigenvalues, $\lambda_{\max}$ and $\lambda_{\min}$, respectively. A comparison of the computed values for $\lambda_{\max}$ and $\lambda_{\min}$ is provided in table \ref{tab:degenerate_stiffness_max_min_eigenvalue}.
