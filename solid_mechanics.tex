...
The reader already familiar with nonlinear solid mechanics and traditional finite element methods is encouraged to proceed onward to chapter \ref{ch:pem_formulation}.
...
At a given material point $\mathbf{X}$, the deformation gradient $\mathbf{F} (\mathbf{X}, t)$ is a linear operator, which may be decomposed into two step-wise operations: a stretching operation $\mathbf{U}(t)$ (or $\mathbf{V}(t)$), and a rotation $\mathbf{R}(t)$, yielding the polar decomposition of $\mathbf{F}$:
\begin{equation}
...
In the context of solid mechanics, the solution of a given boundary value problem entails a complete description of the primary field variable(s). In the semi-discrete case, this refers primarily to the displacement field $\mathbf{u} (\mathbf{X},t_n)$ at all locations $\mathbf{X} \in \mathcal{B}_0$, and at all discrete times $\left\{ t_n \right\}_{n=0}^{N_t}$ beginning at $t_0 = 0$. Under the requirements of compatibility, we presume that the displacement field is a spatially continuous function, whose derivatives up to second order are defined everywhere, i.e. $\mathbf{u} \in \left[ C^2 (\mathcal{B}_0) \right]^d$.

Let us examine a quasi-static solid mechanics model problem of the following form: consider an open domain $\mathcal{B}_0 \subset \mathbb{R}^d$ whose boundary $\partial \mathcal{B}_0$ consists of the partition $\left\{ \Gamma^D_0, \Gamma^N_0 \right\}$. Let $\mathbf{u} = \bar{\mathbf{u}}(t) \, \, \forall \mathbf{X} \in \Gamma^D_0$ constitute a prescribed Dirichlet boundary condition imposed upon the displacement field, and $\mathbf{n} \cdot \boldsymbol{\sigma} = \bar{\mathbf{t}}(t) \, \, \forall \mathbf{X} \in \Gamma^N_0$ be a Neumann boundary condition imposed upon the surface traction. Additionally, let us suppose that an applied body force $\mathbf{b}(t)$ acts upon all points $\mathbf{X} \in \mathcal{B}_0$. For every $t \in \left\{ t_n \right\}_{n=0}^{N_t}$, we should like to determine the displacement field $\mathbf{u} (t) \in \mathcal{S} = \left\{ \mathbf{u} \in \left[ C^2 (\mathcal{B}_0) \right]^d \colon \mathbf{u} = \bar{\mathbf{u}} \, \, \forall \mathbf{x} \in \Gamma^D_0 \right\}$ which satisfies the equations of equilibrium in a point-wise sense:
\begin{equation}
  \rho \mathbf{b} + \nabla_x \cdot \boldsymbol{\sigma} = \mathbf{0} \quad \forall \mathbf{x} \in \mathcal{B}_{t},
\end{equation}
...
\textit{For every $t \in \left\{ t_n \right\}_{n=0}^{N_t}$, find $\mathbf{u}(t) \in \mathcal{S} = \left\{ \mathbf{u} \in \left[H^{1} (\mathcal{B}_0)\right]^d \colon \mathbf{u} = \bar{\mathbf{u}} \, \, \forall \mathbf{X} \in \Gamma^D_0 \right\}$ such that}
\begin{equation}
...
\subsection*{Consistent Linearization of the Weak Form}
	...
	In order to determine the solution $\mathbf{u}^h (t_n) \in \mathcal{S}^h$ satisfying the weak form equations at time $t_n$, the following Newton-Raphson algorithm is employed:
\begin{algorithm}
 \caption{Newton-Raphson Iteration}
 \label{alg:newtonraphson}
 	Compute initial guess $\Delta \mathbf{u}$ \;
	\While{$||\mathbf{R} (\mathbf{u}^h (t_{n-1})+\Delta {\mathbf{u}})|| < \epsilon$}{
 		Assemble $\mathbf{K} = \partial \mathbf{R} / \partial \Delta \mathbf{u}$ \;
 		Update $\Delta \mathbf{u} \leftarrow \Delta \mathbf{u} - \mathbf{K}^{-1}  \mathbf{R}$ \;
 	}
 	Update $\mathbf{u}^h (t_{n}) = \mathbf{u}^h (t_{n-1}) + \Delta {\mathbf{u}}$ \;
\end{algorithm}

	$\mathbf{R}$ denotes the residual (vector), and $\mathbf{K}$ denotes the stiffness matrix -- the Jacobian of the residual with respect to the incremental nodal displacements $\Delta \mathbf{u} = \mathbf{u}^h (t_{n}) - \mathbf{u}^h (t_{n-1})$. The iteration procedure is repeated until the stopping criterion $||\mathbf{R}|| < \epsilon$ is reached, for some specified residual measure $||\cdot||$, and tolerance $\epsilon$.
		
	Given the residual equations $R_{ia} (\mathbf{u}^h) = 0 \, \forall i, \, a$, we may express their first partial derivatives $\partial R_{ia} / \partial \Delta u_{jb}$ with respect to the independent unknowns $\Delta u_{jb}$ as
	\begin{equation}
	  \frac{\partial R_{ia}}{\partial \Delta u_{jb}} =
	  \int_{\Gamma^N_0} \frac{\partial \bar{p}_i}{\partial \Delta u_{jb}}  \phi_a \, dA
	 - \int_{\mathcal{B}_0} \frac{\partial P_{ik}}{\partial \Delta u_{jb}} \phi_{a,k} \, dV,
	\end{equation}
...
\subsection*{Nonlinear Deformation and Incremental Kinematics}
	...
	Otherwise (for hypoelastic material models of the form $\dot{\boldsymbol{\sigma}} = g(\boldsymbol{\sigma}, \mathbf{L})$), the integral of the stress rate over some finite time interval $\Delta t = t_{n} - t_{n-1}$ is required:
	\begin{equation}
		\boldsymbol{\sigma} (t_{n}) = \boldsymbol{\sigma} (t_{n-1}) + \int_{t_{n-1}}^{t_{n}} \dot{\boldsymbol{\sigma}} (\boldsymbol{\sigma}, \mathbf{L}) \, dt.
		\label{eq:stress_update}
	\end{equation}
	The corresponding increment of deformation is denoted $\hat{\mathbf{F}} = \mathbf{F} (t_{n}) \mathbf{F}^{-1} (t_{n-1}) = \hat{\mathbf{R}} \hat{\mathbf{U}}$. In the incremental kinematics algorithm of Rashid \cite{Rashid:93}, the incremental deformation $\hat{\mathbf{F}}$ is applied sequentially, in two distinct steps which consist of: a constant rate of stretching $\mathbf{D} = \frac{1}{\Delta t} \log \hat{\mathbf{U}}$ with $\mathbf{W} = 0$, followed by an impulsive (rigid) rotation $\hat{\mathbf{R}}$. Following this procedure, equation (\ref{eq:stress_update}) becomes:
	\begin{equation}
		\boldsymbol{\sigma} (t_{n}) = \hat{\mathbf{R}} \left[ \boldsymbol{\sigma} (t_{n-1}) + \int_{t_{n-1}}^{t_{n}} \dot{\boldsymbol{\sigma}} (\boldsymbol{\sigma}, \mathbf{D}) \, dt \right] \hat{\mathbf{R}}^T.
	\end{equation}
...
\textit{For a given patch $\mathcal{P} \subset \overline{\mathcal{B}}_0$ and an approximating sequence of functions $u^h \in H^m (\mathcal{B}_0)$: for every $\mathbf{X} \in \mathcal{P}$ there exists an open neighborhood $O$ in $\mathbb{R}^d$ such that}
\begin{equation}
  \lim_{h \rightarrow 0} \sum_{\forall \Omega_e} \int_{\partial \Omega_e} v \, D^\alpha u^h|_{\Omega_e} \mathbf{n} \, da = \mathbf{0}
\end{equation}
\textit{for all test functions $v \in C^{\infty}_0 (O)$, and $| \alpha | \leq m-1$.}
	...
	\begin{equation}
		\int_{\Omega_a \cap \Omega_b} [\![ u^h ]\!] \, da = 0 \quad \forall \Omega_a \cap \Omega_b \neq \emptyset,
	\end{equation}
...
In particular, given a stress field $P_{ij}$ arising from the exact solution $\mathbf{u} \in \mathcal{S}^h$, and for the given boundary conditions $\bar{p}_i = P_{ij} N_j$ and $\rho_0 b_i = - P_{ij,j}$, we may express the residual equations as
\begin{equation}
