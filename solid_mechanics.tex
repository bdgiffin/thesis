...
\section{A Model Solid Mechanics Boundary Value Problem}
...
Finite element methods (FEM) are predicated on the idea that a problem domain $\mathcal{B}$ can be discretized into a finite number of simpler sub-domains $\omega_e \subset \mathcal{B}$, individually called elements, and collectively referred to as a ``mesh.'' The basis functions are assumed to be low-order polynomials within each element which possess compact support over a given patch of elements. The traditional finite element method assumes these basis functions to be $C^0$ continuous at element boundaries, yielding a priori satisfaction of compatibility.
