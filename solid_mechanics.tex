...
By comparison, non-conforming finite element methods admit more general functions $\mathbf{u}^h \in \mathcal{S}^h \not\subset C^0$. Such spaces of functions must satisfy certain additional requirements. Namely, Stummel has proposed in \cite{Stummel:79} the necessary and sufficient conditions for convergence of a non-conforming finite element method, necessitating passage of the \textit{generalized patch test}:
...
\textit{for all test functions $v \in C^{\infty}_0 (O)$, and $| \alpha | \leq k-1$.}
	...
	Shi later proposed the F-E-M test in \cite{Shi:87} as an alternative, sufficient condition for convergence, with corresponding interface requirement (the F-test):
	\begin{equation}
...
\mathcal{V}^h} \frac{a(u^h,v^h)}{||u^h|| \, ||v^h||} > 0.
	...
	\end{equation}
	\textit{Existence} of a solution further depends upon the condition that $a(\cdot, \cdot)$ be bounded:
	\begin{equation}
